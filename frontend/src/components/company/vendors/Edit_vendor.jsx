...
function Edit_vendor () {
    ...
    const get_vendor_details = () =>{
        axios.get(`${config.base_url}/get_vendor_details/${id}/${ID}/`).then((res)=>{
            if(res.data.status){
                ...
                var company = res.data.com;
                setFirstName(vendor.First_name);
                setTitle(vendor.Title);
                setLastName(vendor.Last_name);
                setCompany(vendor.Company_Name);
                setLocation(vendor.Location);
                setPlaceOfSupply(vendor.Place_of_supply);
                setGstType(vendor.GST_Treatment);
                setGstIn(vendor.GST_Number);
                setPanNo(vendor.Pan_Number);
                setOBalType(vendor.Opening_balance_type);
                setOBal(vendor.Opening_balance);
                setCreditLimit(vendor.Credit_limit);
                setPaymentTerm(vendor.payment_terms);
                setEmail(vendor.Vendor_email);
                setWebsite(vendor.Website);
                setMobile(vendor.Mobile);
                setBStreet(vendor.Billing_street);
                setBCity(vendor.Billing_city);
                setBCountry(vendor.Billing_country);
                setBPincode(vendor.Billing_pincode);
                setBState(vendor.Billing_state);
                setSCity(vendor.Shipping_city);
                setSCountry(vendor.Shipping_country);
                setSPincode(vendor.Shipping_pincode);
                setSState(vendor.Shipping_state);
                setSStreet(vendor.Shipping_street);
              }
              setVendor(vendor);
              setCompanyn(company);
        }).catch((err) =>{
    ...
    const handleSubmit = (e) => {
      ...
      axios
        .put(`${config.base_url}/update_vendor/`, dt)
        .then((res) => {
